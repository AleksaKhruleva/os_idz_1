 ...
 *   it to the second process through a named pipe;
...
char *fifo1_n = "/tmp/idz1_5_1";    //  fifo name for process-1 & process-2
char *fifo2_n = "/tmp/idz1_5_2";    //  fifo name for process-2 & process-3
...
void usage() {
    printf("Usage:\n");
    printf("\tidz1_5 -i input_file_name -o output_file_name\n");
    exit(0);
}
...
void mission_2() {
    ...
    break_on_cond_1(fifo2_d < 0, "fifo open(%s) error: %i. %s\n", fifo2_n, errno);
    //  read data from fifo
    ssize_t data_len = read(fifo2_d, buff, buff_size);
    break_on_cond_1(data_len < 0, "fifo read(%s) error: %i. %s\n", fifo2_n, errno);
